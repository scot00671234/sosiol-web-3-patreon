...
import { createUSDCTransferTransaction, getWorkingConnection } from '../utils/solana';
...
const TipButton: FC<TipButtonProps> = ({ creatorWallet, creatorName, onSuccess }) => {
  ...
  const handleTip = async (amount: number) => {
    ...
    try {
      ...
      // Wait for confirmation using working RPC connection
      const workingConnection = getWorkingConnection();
      ...
      try {
        ...
        // Check if transaction actually succeeded despite confirmation failure
        const txInfo = await workingConnection.getTransaction(signature);
        if (txInfo && !txInfo.meta?.err) {
          console.log('✅ Transaction actually succeeded, confirmation just failed');
        } else {
