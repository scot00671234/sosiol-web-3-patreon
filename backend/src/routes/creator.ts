...
router.get('/:walletAddress/dashboard', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`🔍 Dashboard data for ${walletAddress}:`, {
      ...
      tips: tips.map((t: any) => ({ id: t.id, amount: t.amountUSDC, status: t.status, from: t.fromWallet }))
    });
...
router.get('/:walletAddress/wallet-info', async (req: Request, res: Response) => {
  try {
    ...
    const totalTipsFromDB = allTips
      .filter((tip: any) => tip.status === 'completed')
      .reduce((sum: number, tip: any) => sum + tip.amountUSDC, 0);


    res.json({
      walletAddress,
      totalTipsReceived: totalTipsFromDB,
      recentTips: allTips.slice(0, 10).map((tip: any) => ({
        id: tip.id,
